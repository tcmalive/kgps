...
public class WebGeocode {
	...
	public static List<Address> getFromLocation(double lat, double lon,
		...
		Log.d(Common.LOG_TAG, "" + responseArray.length() + " result(s)");
		...
		for (int i = 0; i < responseArray.length() && i < maxResults - 1; i++) {
			...
			try {
				...
				addy.setPostalCode(jsl.getJSONObject("PostalCode").getString(
					"PostalCodeNumber"));
				addy.setThoroughfare(jsl.getJSONObject("Thoroughfare").getString(
					"ThoroughfareName"));
	...
	public static List<Address> getFromLocationName(String locationAddress,
		int maxResults) {
		String urlStr = "http://maps.google.com/maps/geo?q=" + locationAddress
			+ "&output=json&sensor=false";
		...
		for (int i = 0; i < responseArray.length() && i < maxResults - 1; i++) {
			...
			try {
				...
				jsl = getJSONObject(jsl, "AddressDetails");
				if (jsl != null) {
					JSONObject jslCountry = getJSONObject(jsl, "Country");
					if (jslCountry != null) {
						addy.setCountryName(jslCountry.getString("CountryName"));
						addy.setCountryCode(jslCountry.getString("CountryNameCode"));
					}
				}
				jsl = getJSONObject(jsl, "AdministrativeArea");
				if (jsl != null) {
					addy.setAdminArea(jsl.getString("AdministrativeAreaName"));
				}

				jsl = getJSONObject(jsl, "SubAdministrativeArea");
				if (jsl != null) {
					addy.setSubAdminArea(jsl.getString("SubAdministrativeAreaName"));
				}

				jsl = getJSONObject(jsl, "Locality");
				if (jsl != null) {
					addy.setLocality(jsl.getString("LocalityName"));
				}

				JSONObject jslPostalCode = getJSONObject(jsl, "PostalCode");
				if (jslPostalCode != null) {
					addy.setPostalCode(jslPostalCode.getString("PostalCodeNumber"));
				}
				JSONObject jslThoroughfare = getJSONObject(jsl, "Thoroughfare");
				if (jslThoroughfare != null) {
					addy.setThoroughfare(jslThoroughfare.getString("ThoroughfareName"));
				}
	...
	private static JSONObject getJSONObject(JSONObject jsl, String objectName) {
		JSONObject json = null;
		if (jsl == null)
			return json;

		try {
			json = jsl.getJSONObject(objectName);
		} catch (JSONException e) {
			Log.d(Common.LOG_TAG, Common.getString(e));
		}
