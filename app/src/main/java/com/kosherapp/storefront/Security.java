...
import com.kosherapp.util.Base64;
import com.kosherapp.util.Base64DecoderException;
...
public class Security {
	 ...
	 * message and we will re-generate a new nonce. This has to be "static" so that
	 * the {@link BillingReceiver} can check if a nonce exists.
	 */
	...
	public static ArrayList<VerifiedPurchase> verifyPurchase(String signedData,
		...
		if (!TextUtils.isEmpty(signature)) {
			 ...
			 * adversary to replace the public key with one of their own and then fake
			 * messages from the server.
			 * 
			 * Generally, encryption keys / passwords should only be kept in memory long
			 * enough to perform the operation they need to perform.
			 */
	...
	public static PublicKey generatePublicKey(String encodedPublicKey) {
		try {
			byte[] decodedKey = com.kosherapp.util.Base64.decode(encodedPublicKey);
