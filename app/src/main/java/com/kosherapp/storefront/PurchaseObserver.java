...
public abstract class PurchaseObserver {
	...
	/**
	 * This is the callback that is invoked when an item is purchased, refunded, or
	 * canceled. It is the callback invoked in response to calling
	 ...
	 * purchase the item. The possible response codes are: RESULT_OK means that the
	 * order was sent successfully to the server. The onPurchaseStateChange() will
	 * be invoked later (with a purchase state of PURCHASED or CANCELED) when the
	 * order is charged or canceled. This response code can also happen if an order
	 * for a Market-managed item was already sent to the server.
	 ...
	 * RestoreTransactions request that we made. A response code of RESULT_OK means
	 * that the request was successfully sent to the server.
	 */
	...
	void startBuyPageActivity(PendingIntent pendingIntent, Intent intent) {
		if (mStartIntentSender != null) {
			...
			// This is on Android version 1.6. The in-app buy page activity must be on
			// its
