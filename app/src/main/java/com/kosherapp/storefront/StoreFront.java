...
package com.kosherapp.storefront;

import com.kosherapp.Common;
import com.kosherapp.R;
import com.kosherapp.R.id;
import com.kosherapp.R.layout;
import com.kosherapp.R.string;
import com.kosherapp.storefront.BillingService.RequestPurchase;
import com.kosherapp.storefront.BillingService.RestoreTransactions;
import com.kosherapp.storefront.Consts.PurchaseState;
import com.kosherapp.storefront.Consts.ResponseCode;
 ...
 */
public class StoreFront extends Activity implements OnClickListener,
	OnItemSelectedListener {
	private static final String								TAG																													= "StoreFront";

	public final static int												REQUEST_CODE																				= 987654321;

	/**
	 * Used for storing the log text.
	 */
	private static final String								LOG_TEXT_KEY																				= "STOREFRONT_LOG_TEXT";

	/**
	 * The SharedPreferences key for recording whether we initialized the database.
	 * If false, then we perform a RestoreTransactions request to get all the
	 * purchases for this user.
	 */
	private static final String								DB_INITIALIZED																		= "db_initialized";

	private StoreFrontPurchaseObserver	mStoreFrontPurchaseObserver;
	 ...
	 * that the product can be purchased only once per user (such as a new level in
	 * a game). The purchase is remembered by Android Market and can be restored if
	 * this application is uninstalled and then re-installed. UNMANAGED is used for
	 * products that can be used up and purchased multiple times (such as poker
	 * chips). It is up to the application to keep track of UNMANAGED products for
	 * the user.
	 */
	 ...
	 */
	private class StoreFrontPurchaseObserver extends PurchaseObserver {
		public StoreFrontPurchaseObserver(Handler handler) {
			super(StoreFront.this, handler);
		}
		...
		public void onRequestPurchaseResponse(RequestPurchase request,
			...
			if (responseCode == ResponseCode.RESULT_OK) {
				...
				StoreFront.this.mOkButton.setEnabled(true);
	...
	private static ArrayList<CatalogEntry>	CATALOG	= new ArrayList<CatalogEntry>();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		String[] skus = data.getStringArrayExtra(Common.INTENT_KEY_SKUS);
		int[] nameIds = data.getIntArrayExtra(Common.INTENT_KEY_NAME_IDS);
		String[] manageds = data.getStringArrayExtra(Common.INTENT_KEY_MANAGEDS);

		if (skus != null && nameIds != null && manageds != null)
			addCatalogItems(skus, nameIds, manageds);
		fillCatalog();

		mHandler = new Handler();
		mStoreFrontPurchaseObserver = new StoreFrontPurchaseObserver(mHandler);
		...
		ResponseHandler.register(mStoreFrontPurchaseObserver);
		if (!mBillingService.checkBillingSupported()) {
			showDialog(DIALOG_CANNOT_CONNECT_ID);
		}
	}

	private void addCatalogItems(String[] skus, int[] nameIds, String[] manageds) {
		if (!(skus.length == nameIds.length && nameIds.length == manageds.length))
			throw new IllegalArgumentException();

		for (int i = 0; i < skus.length; i++) {
			String sku = skus[i];
			int nameId = nameIds[i];
			Managed managed = Managed.valueOf(manageds[i].toUpperCase());

			CATALOG.add(new CatalogEntry(sku, nameId, managed));
		}
	}

	private void fillCatalog() {
		// CATALOG.addAll(Arrays.asList(new CatalogEntry[] {
	...
	protected void onStop() {
		super.onStop();
		ResponseHandler.unregister(mStoreFrontPurchaseObserver);
	}
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		outState.putString(LOG_TEXT_KEY, Html
			.toHtml((Spanned) mLogTextView.getText()));
	}
	...
	private Dialog createDialog(int titleId, int messageId) {
		...
		builder.setTitle(titleId).setIcon(android.R.drawable.stat_sys_warning)
			.setMessage(messageId).setCancelable(false).setPositiveButton(
				android.R.string.ok, null).setNegativeButton(R.string.learn_more,
	 ...
	 * The pattern "%lang%" will be replaced by the device's language code and the
	 * pattern "%region%" will be replaced with the device's country code.
	 * 
	 * @param str
	...
	private void setupWidgets() {
		...
		mOkButton = (Button) findViewById(R.id.storeFrontOKButton);
		mOkButton.setEnabled(false);
		mOkButton.setOnClickListener(this);

		mCancelButton = (Button) findViewById(R.id.storeFrontCancelButton);
	 ...
	 * data. We do not want to do this on every startup, rather, we want to do only
	 * when the database needs to be initialized.
	 */
	private void restoreDatabase() {
		...
		if (!initialized) {
			...
			Toast.makeText(this, R.string.restoring_transactions, Toast.LENGTH_LONG)
				.show();
		}
	}

	/**
	 * Creates a background thread that reads the database and initializes the set
	 * of owned items.
	 */
	 ...
	 * and then adds those items to the set of owned items in the main UI thread.
	 */
	...
	public void onItemSelected(AdapterView<?> parent, View view, int position,
		long id) {
		mItemName = getString(CATALOG.get(position).nameId);
		mSku = CATALOG.get(position).sku;
	}
	...
	private static class CatalogAdapter extends ArrayAdapter<String> {
		...
		public boolean isEnabled(int position) {
			...
			CatalogEntry entry = mCatalog.get(position);
