...
public class PurchaseDatabase {
	...
	/**
	 * Inserts a purchased product into the database. There may be multiple rows in
	 * the table for the same product if it was purchased multiple times or if it
	 * was refunded.
	 * 
	 * @param orderId
	...
	/**
	 * Updates the quantity of the given product to the given value. If the given
	 * value is zero, then the product is removed from the table.
	 * 
	 * @param productId
	...
	public synchronized int updatePurchase(String orderId, String productId,
		...
		Cursor cursor = mDb.query(PURCHASE_HISTORY_TABLE_NAME, HISTORY_COLUMNS,
			HISTORY_PRODUCT_ID_COL + "=?", new String[] { productId }, null, null, null,
			null);
		if (cursor == null) {
			return 0;
		}
	...
	public Cursor queryAllPurchasedItems() {
		return mDb.query(PURCHASED_ITEMS_TABLE_NAME, PURCHASED_COLUMNS, null, null,
			null, null, null);
	}
	...
	private class DatabaseHelper extends SQLiteOpenHelper {
		...
		private void createPurchaseTable(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + PURCHASE_HISTORY_TABLE_NAME + "("
				+ HISTORY_ORDER_ID_COL + " TEXT PRIMARY KEY, " + HISTORY_STATE_COL
				+ " INTEGER, " + HISTORY_PRODUCT_ID_COL + " TEXT, "
				+ HISTORY_DEVELOPER_PAYLOAD_COL + " TEXT, " + HISTORY_PURCHASE_TIME_COL
				+ " INTEGER)");
