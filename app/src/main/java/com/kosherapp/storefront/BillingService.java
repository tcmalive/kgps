...
import com.kosherapp.storefront.Security.VerifiedPurchase;
...
public class BillingService extends Service implements ServiceConnection {
	 ...
	 * have not yet received a response code. The HashMap is indexed by the request
	 * Id that each request receives when it executes.
	 */
	...
	class RequestPurchase extends BillingRequest {
		...
		protected void responseCodeReceived(ResponseCode responseCode) {
			ResponseHandler
				.responseCodeReceived(BillingService.this, this, responseCode);
		}
	}
	 ...
	 * identifiers back to the MarketBillingService, which ACKs them to the server.
	 ...
	 * server responds with the purchase information, encoded as a JSON string, and
	 ...
	 * acknowledging that an order was sent to the server. This is NOT used for any
	 * purchase state changes. All purchase state changes are received in the
	 * {@link BillingReceiver} and passed to this service, where they are handled
	 * in {@link #purchaseStateChanged(int, String, String)}.
	 * 
	 * @param requestId
	 *         a number that identifies a request, assigned at the time the request
	 *         was made to Android Market
	 * @param responseCode
